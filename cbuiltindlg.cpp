    ...
    : QDialog(parent)
{
    ...
    colorPushBtna = new QPushButton (QStringLiteral("字顏色對話盒"));
    ...
    gridLayout->addWidget (colorPushBtna,2,2,1,1);
    ...
    connect(colorPushBtna,SIGNAL(clicked()),this,SLOT(doTextcolor()));
}

cbuiltindlg::~cbuiltindlg() {}
void cbuiltindlg:: doTextcolor ()
{
    QPushButton* btm = qobject_cast<QPushButton*>(sender());
    if (btm == colorPushBtna){
    QPalette palette=displayTextEdit->palette();
    const QColor& color=
        QColorDialog::getColor(palette.color(QPalette::Text),this, QStringLiteral("設定字顏色"));
    if(color.isValid())
    {
...
void cbuiltindlg:: doPushbtn ()
{
    ...
    if (btn == filePushBtn)
    {
        QString fileName = QFileDialog::getOpenFileName (this,QStringLiteral("打開檔案"),QStringLiteral("."),QStringLiteral("任何檔案(*.*);;文字檔(*.txt);;XML檔(*.xml)"));
